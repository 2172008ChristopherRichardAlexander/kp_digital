      ...
      <div class="row">
        <div class="col">
          <h3 class="keterangan-website my-2">
            Pengaturan Batch Pengajuan Topik
          </h3>
        </div>
        <div class="button-keterangan-website col-3 text-right">
          <b-button
            class="btn-form"
            v-b-modal.modal-tambah-batch
            :disabled="loading"
          >
            {{ loading ? "" : "Buka Batch Baru" }}
            <ring-loader
              class="loading-page"
              color="white"
              :size="25"
...
export default {
  ...
  methods: {
    ...
    getBatch() {
      ...
      Axios.get(`${config.apiUrl}/batch-list/1`)
        .then((response) => {
    ...
    async tambahBatch() {
      ...
      if (this.$v.$invalid) {
        ...
        var status = await this.cekStatus(this.aktifasiBatch);
        Axios.post(
          `${config.apiUrl}/batch`,
          {
