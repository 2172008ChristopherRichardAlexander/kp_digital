<template>
  <b-container>
    <div class="form-penilaian-mahasiswa">
      ...
      <div class="penilaian-mahasiswa">
        <b-table
          sticky-header
          class="table-penilaian-mahasiswa"
          :items="list_mahasiswa"
          :fields="fields"
          ref="tableListMahasiswa"
          hover
          striped
          responsive="sm"
          :busy="isBusyPenilaian"
          show-empty
        ...
        >
          ...
          <template v-slot:cell(nilai_koor_kp)="data">{{
            data.item.nilai_koor_kp ? data.item.nilai_koor_kp : "Belum di Nilai"
          }}</template>
          <template v-slot:cell(penilaian)="data">
            <router-link
              :to="`/sidang/penilaian/${acakAcak(data.item.id_sidang, 0)}`"
              class="btn btn-form"
              >{{ data.item.nilai_koor_kp ? "Lihat" : "Nilai" }}</router-link
            >
          </template>
        </b-table>
        <b-row>
...
export default {
  data() {
    return {
      ...
      fields: [
        { key: "topik.id_pengaju", label: "NRP" },
        "nama",
        { key: "topik.judul_topik", label: "Judul Topik" },
        { key: "nilai_koor_kp", label: "Nilai" },
        "penilaian",
      ],
      loading: false,
      show: true,
      isBusyPenilaian: true,
      id_batch: null,
      pilihan_batch: [],
      id_semester: null,
      semester: null,
  ...
  methods: {
    ...
    getMahasiswa() {
      this.isBusyPenilaian = true;
      this.loading = true;
      const formData = new FormData();
      formData.append("id_batch", this.id_batch);
      Axios.post(`${config.apiKoorKpUrl}/sidang/daftar-mahasiswa`, formData, {
        headers: {
        ...
        .then((response) => {
          if (!response.data.message) {
            this.list_mahasiswa = response.data.data;
    ...
    changeDataSidang() {
      // console.log(this.list_mahasiswa);
      // if (!this.list_mahasiswa)
      //   for (let index = 0; index < this.list_mahasiswa.length; index++) {
      //     const element = this.list_mahasiswa[index];
      //     // Axios.get(
      ...
      //     this.$refs.tableListMahasiswa.refresh();
      //     // });
      //     if (index == this.list_mahasiswa.length - 1)
      //       this.$refs.tableListMahasiswa.refresh();
      //   }
      this.isBusyPenilaian = false;
    },
    acakAcak(isi, status) {
